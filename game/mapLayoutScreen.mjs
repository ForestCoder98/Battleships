...
import { retrieveLanguage } from "../dictionary.mjs";
...
function createMapLayoutScreen() {
    ...
    const MapLayout = {
        ...
        draw: function (dr) {
            ...
            let output = `${ANSI.TEXT.BOLD}${ANSI.COLOR.YELLOW}${retrieveLanguage().NAVIGATE_SHIP_PHASE}\n\n${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}`;
            ...
            output += `${ANSI.TEXT.BOLD}${ANSI.COLOR.YELLOW}${retrieveLanguage().CONTROL_INPUT_PROMPT}${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}\n`;
            output += retrieveLanguage().MOVEMENT_KEY;
            output += retrieveLanguage().ROTATE_SHIP_KEY;
            output += retrieveLanguage().CONFIRM_SHIP_PLACEMENT_KEY;

            output += `\n${ANSI.TEXT.BOLD}${ANSI.COLOR.YELLOW}${retrieveLanguage().SHIPS_TO_PLACE_PROMPT}${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}\n`;
            this.ships.forEach((ship, index) => {
                const status = index < this.currentShipIndex ? '✓' :
                    index === this.currentShipIndex ? '>' : ' ';
                output += `${status} ${ship.id} (${ship.size}${retrieveLanguage().SPACES_PROMPT})\n`;
            });
